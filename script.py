# IMPORTANT NOTE : use this script for each browser ONLY when the browser is closed.
...
def remove_edge_history(search_str: str):
    localappdata = os.getenv("LOCALAPPDATA")
    profiles_path = os.path.join(localappdata, "Microsoft", "Edge", "User Data")
    ...
    profile_dirs = [d for d in os.listdir(profiles_path) if os.path.isdir(os.path.join(profiles_path, d))]
    if not profile_dirs:
        print("Found 0 Edge profile(s).")
        return

    print(f"Found {len(profile_dirs)} Edge profile(s).")
...
if __name__ == "__main__":
    browser = input("Enter the browser (c for chrome, f for firefox, e for edge and a for all): ").lower().strip()
    assert browser and all(char in ('c', 'f', 'e', 'a', ' ') for char in browser), "Invalid input"
    term = input("Enter search string to remove from browsing history: ")
    assert term, "Search string can't be empty"
    if browser == 'a':
        remove_firefox_history(term)
        remove_chrome_history(term)
        remove_edge_history(term)
    else:
        if 'f' in browser:
            remove_firefox_history(term)
        if 'c' in browser:
